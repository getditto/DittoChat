...
describe('useChatUser Slice', () => {
  ...
  describe('updateUser', () => {
    it('fetches current user, merges data, and updates DB', async () => {
      ...
      mockDitto.store.execute.mockResolvedValueOnce({
        items: [{ value: currentUser }],
      })

      await store
        .getState()
        .updateUser({ _id: 'user-update', name: 'New Name' })
    ...
    it('returns early when user not found', async () => {
      ...
      await store
        .getState()
        .updateUser({ _id: 'nonexistent', name: 'New Name' })
  ...
  describe('toggleRoomSubscription', () => {
    ...
    it('toggleRoomSubscription runs unconditionally', async () => {
  ...
  describe('Observer callbacks', () => {
    ...
    it('sets up allUsers observer and subscription', () => {
      ...
      const allUsersCall = subscriptionCalls.find(
        (call: unknown[]) =>
          (call[0] as string).includes('SELECT * FROM users') &&
          call.length === 1,
      )
      ...
      const allUsersObserverCall = observerCalls.find(
        (call: unknown[]) =>
          (call[0] as string).includes('SELECT * FROM users') &&
          !(call[0] as string).includes('WHERE'),
      )
  ...
  describe('Edge cases with null ditto', () => {
    ...
    it('updateUser returns early when _id is missing', async () => {
      await store
        .getState()
        .updateUser({ name: 'Test' } as unknown as ChatUser & { _id: string })
