...
describe('useMessages Slice', () => {
  ...
  describe('createMessage', () => {
    ...
    it('creates message with mentions and updates user mentions', async () => {
      ...
      await store
        .getState()
        .createMessage(mockRoom, 'Hello @User1 @User2', mentions)
  ...
  describe('saveDeletedMessage', () => {
    it('archives original and creates deletion marker for text', async () => {
      ...
      await store
        .getState()
        .saveDeletedMessage(originalMsg as Message, mockRoom)
    ...
    it('handles image deletion', async () => {
      ...
      await store
        .getState()
        .saveDeletedMessage(message as Message, mockRoom, 'image')
    ...
    it('handles file deletion', async () => {
      ...
      await store
        .getState()
        .saveDeletedMessage(message as Message, mockRoom, 'file')
  ...
  describe('saveEditedTextMessage', () => {
    it('archives old message and inserts new version', async () => {
      ...
      await store
        .getState()
        .saveEditedTextMessage(originalMsg as Message, mockRoom)
  ...
  describe('Message reactions', () => {
    describe('addReactionToMessage', () => {
      it('updates the message optimistically and in DB', async () => {
        ...
        expect(mockDitto.store.execute).toHaveBeenCalledWith(
          expect.stringContaining(
            `UPDATE ${mockRoom.messagesId} SET reactions`,
          ),
          expect.objectContaining({
            id: messageId,
            reactions: [reaction],
        ...
        const updatedMsg =
          store.getState().messagesByRoom[mockRoom._id][0].message
      ...
      it('throws when message not found', async () => {
        ...
        await expect(
          store
            .getState()
            .addReactionToMessage(message as Message, mockRoom, reaction),
      ...
      it('should not add reaction when canAddReaction permission is false', async () => {
        ...
        await state.addReactionToMessage(
    ...
    describe('removeReactionFromMessage', () => {
      ...
      it('throws when message not found', async () => {
        ...
        await expect(
          store
            .getState()
            .removeReactionFromMessage(message as Message, mockRoom, reaction),
        ).rejects.toThrow('Message not found')
      })
    ...
    describe('updateMessageReactions', () => {
      it('handles errors and rolls back', async () => {
        ...
        store.setState({
          messagesByRoom: {
            [mockRoom._id]: [
              { message: message as Message, id: message._id, user: null },
            ],
          },
        })
        ...
        await store
          .getState()
          .updateMessageReactions(message as Message, mockRoom, newReactions)
      ...
      it('throws when message not found', async () => {
        ...
        await expect(
          store
            .getState()
            .updateMessageReactions(message as Message, mockRoom, []),
        ).rejects.toThrow('Message not found')
      })
  ...
  describe('fetchAttachment', () => {
    it('handles progress and completion', async () => {
      ...
      mockDitto.store.fetchAttachment.mockImplementation(
        (token: AttachmentToken, cb: (event: AttachmentFetchEvent) => void) => {
    ...
    it('returns error when ditto is null', () => {
      ...
      expect(onComplete.mock.calls[0][0].error.message).toBe(
    ...
    it('returns error when token is null', () => {
      ...
      store
        .getState()
        .fetchAttachment(
          null as unknown as AttachmentToken,
          onProgress,
          onComplete,
        )
  ...
  describe('messagesPublisher', () => {
    ...
    it('does not create duplicate subscription', async () => {
      ...
      const firstCallCount =
        mockDitto.sync.registerSubscription.mock.calls.length
  ...
  describe('Message update logic (handleMessageUpdate)', () => {
    ...
    describe('Archived messages (edited/deleted)', () => {
      ...
      it('updates existing archived message when it already exists', async () => {
        ...
        observerCallback({
          items: [{ value: updatedArchivedMessage }],
        } as unknown as Partial<QueryResult<Message>>)
    ...
    describe('Regular messages (new/updated)', () => {
      ...
      it('handles multiple messages in single observer callback', async () => {
        ...
        observerCallback({
          items: messages.map((value) => ({ value })),
        } as unknown as Partial<QueryResult<Message>>)
      ...
      it('handles empty result items', async () => {
        observerCallback({ items: [] } as unknown as Partial<
          QueryResult<Message>
        >)
    ...
    describe('Complex scenarios', () => {
      it('handles mix of new, updated, and archived messages', async () => {
        ...
        observerCallback({
          items: originalMessages.map((value) => ({ value })),
        } as unknown as Partial<QueryResult<Message>>)
        ...
        observerCallback({
          items: mixedUpdate.map((value) => ({ value })),
        } as unknown as Partial<QueryResult<Message>>)
