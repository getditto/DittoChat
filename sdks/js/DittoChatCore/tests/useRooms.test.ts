...
describe("useRooms Slice", () => {
  ...
  describe("Initialization", () => {
    ...
    it("initializes subscriptions for DM rooms", () => {
      expect(mockDitto.sync.registerSubscription).toHaveBeenCalledWith(
        expect.stringContaining("SELECT * from dm_rooms where (array_contains(participants, :userId))"),
        { userId: "test-user-id" }
      );
    });
  ...
  describe("createRoom", () => {
    ...
    it("handles errors gracefully", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Error creating rooms:", expect.any(Error));
  ...
  describe("createDMRoom", () => {
    ...
    it("handles errors gracefully", async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith("Error creating dm_rooms:", expect.any(Error));
      expect(result).toBeUndefined();
      consoleSpy.mockRestore();
    });
  ...
  describe("Observer callbacks", () => {
    describe("Rooms observer", () => {
      ...
      it("filters and replaces rooms by collectionId", () => {
        ...
        expect(store.getState().rooms).toHaveLength(2);
        expect(store.getState().rooms.find(r => r.collectionId === "dm_rooms")).toBeDefined();
        expect(store.getState().rooms.find(r => r.collectionId === "rooms")).toBeDefined();
      });
    });
    ...
    describe("DM rooms observer", () => {
      it("updates local state when DM rooms observer receives new rooms", () => {
        ...
        expect(store.getState().rooms[0].participants).toEqual(["user-1", "user-2"]);
      });
    });
  });
});
