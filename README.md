...
Open an existing project or create a new one in Android Studio (**File → New → New Project → Empty Activity**). Choose **Kotlin** as the language and set the minimum SDK to **API 24 (Android 7.0)** or higher (required by Ditto SDK v5).
...
2. In your **app-level** `build.gradle`, add the Ditto SDK:
   ...
   dependencies {
       implementation "com.ditto:ditto-kotlin-android:5.0.1"  // Check docs.ditto.live for the latest version
   }
   ```
   ...
   [libraries]
   ditto = { module = "com.ditto:ditto-kotlin-android", version.ref = "ditto" }
   ```
...
> **Important:** Replace `YOUR_DATABASE_ID` and `YOUR_CLOUD_ENDPOINT` with your portal credentials. Store secrets securely — use `BuildConfig` fields or a `local.properties` file that is excluded from version control.

> **Authentication:** If your app uses Online Playground or a custom auth provider, set an `auth.expirationHandler` before calling `sync.start()`:
>
> ```kotlin
...
2. Both apps must be configured with the **same Database ID** from the Ditto Portal.
...
| `DittoException` on startup | Invalid App ID or endpoint | Double-check your credentials in the [Ditto Portal](https://portal.ditto.live) |
| Messages appear only on one device | Different App IDs | Ensure both devices use the same App ID |
| Permissions prompt doesn't appear (Android) | `requestPermissions` not called | Call `DittoSyncPermissions.missingPermissions()` before `sync.start()` |
...
- **Go to production** — Replace the development `auth.expirationHandler` with a production token provider. See the [Ditto Cloud Authentication docs](https://docs.ditto.live/auth-and-authorization/cloud-authentication) for a full walkthrough.
